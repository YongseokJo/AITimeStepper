...
must_haves:
  truths:
    ...
    - "Duration warning issued when duration is set"
...
def run_training(config: Config) -> None:
    ...
    # === SECTION 6: UNSUPPORTED FEATURE WARNINGS (NEW) ===
    ...
    if config.duration is not None:
        warnings.warn(
            ...
            "in two-phase training. Training will run for {config.epochs} epochs instead.",
            UserWarning,
        )
...
4. Duration warning issued when `duration` is set
5. Training summary printed after completion
6. Checkpoint contract preserved - simulation mode loads new checkpoints
7. W&B cleanup still runs (`wandb.finish()`)
</verification>

<success_criteria>
- run_training() delegates to run_two_phase_training() for training loop
- Existing Config fields (history_len, feature_type, num_orbits, duration) still supported
- Multi-orbit warning issued (graceful degradation, not error)
- Duration warning issued (graceful degradation, not error)
