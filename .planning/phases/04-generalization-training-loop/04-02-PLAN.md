...
must_haves:
  truths:
    ...
    - "Trajectory particles are never modified during replay"
    - "Training actually improves model performance over iterations"
  artifacts:
...
3. **MockModel class (copy from test_trajectory_collection.py):**
   ```python
...
4. **TrainableMockModel class (full implementation):**
   ```python
...
- MockModel class defined with dynamic input dimension handling
- TrainableMockModel class defined with full implementation including dynamic resize
...
- TestEvaluateMinibatch with 4 test cases (including particle immutability test)
- All tests pass
  </done>
</task>
...
```
  ...
  <done>
- TestGeneralizeOnTrajectory with 8 test cases
- Tests cover: return structure, convergence, empty trajectory, small trajectory, max iterations, metrics
- test_converges_after_training verifies actual learning behavior
- test_trajectory_immutability_during_training verifies particles unchanged after training
- All tests pass
  </done>
</task>
...
# Check line count
wc -l tests/test_generalization_training.py | awk '{if ($1 >= 200) print "Line count OK: " $1 " lines"; else print "FAIL: Only " $1 " lines"}'
...
<success_criteria>
1. `tests/test_generalization_training.py` exists with 200+ lines
2. TestSampleMinibatch class with 4+ test methods
3. TestEvaluateMinibatch class with 4+ test methods (including particle immutability)
4. TestGeneralizeOnTrajectory class with 8+ test methods
5. All tests pass with pytest
6. Tests cover edge cases: empty trajectory, small trajectory, max iterations
7. test_converges_after_training validates actual learning behavior
8. test_trajectory_immutability tests verify particles unchanged during replay
</success_criteria>
